...
$query = mysqli_query($connection,'INSERT INTO categorias (categoria_descricao_ingles, categoria_descricao_japones, departamento_id, categoria_tela_principal, categoria_criacao) VALUES ("'.$categoria_descricao_ingles.'", "'.$categoria_descricao_japones.'", "'.$departamento_id.'", "'.$categoria_tela_principal.'", "'.date($now).'")');
...
if (isset($_FILES["uploadFile"]["name"])) {
	...
	if ($img){
		...
		$imagem_nova = 'CAT_'.$id_novo;

		$query3 = mysqli_query($connection,"UPDATE categorias set categoria_imagem='".$imagem_nova."' WHERE id=".$id_novo);
