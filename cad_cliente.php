...
//veio do checkout do carrinho de compra e vai para o review.php apos o cadastro
$vapara = '';
...
<body>
	...
	<div class="login-page">
		<div class="container"> 
			<h3 class="w3ls-title w3ls-title1"><?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=15"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?></h3>  
            ...
            <script>
				...
				<form action="processa/proc_cad_cliente.php" method="post" name="form_cliente" id="form_cliente">
                    <h4><?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=16"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?></h4>  
						<input type="text" class="form-control" id="cliente_nome" name="cliente_nome" placeholder="<?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=17"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?>" required=""
                        >
					...
					<input type="submit" onclick="verificaCliente();" value="<?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=29"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?>">
					<div class="forgot-grid">
						<h6><label class="checkbox"><input type="checkbox" name="checkbox"><?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=30"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?></label>
						<div class="forgot">
							<a href="recuperar_senha.php" onclick="<?php echo $loading;?>"><?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=31"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?></a></h6>
						</div>
			...
			<h6><?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=32"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?><a href="login.php" onclick="<?php echo $loading;?>"><?php $query =mysqli_query($connection,"SELECT mensagem_".$msg_idioma." as mensagem FROM mensagens WHERE id=33"); while($line = mysqli_fetch_assoc($query)){ echo $line["mensagem"]; } ?> »</a> </h6>  
		</div>
	</div>
	<!-- //sign up-page --> 
